...
class usuarios
{
    ...
    const MAIL_BIENVENIDO = "Bienvenido a Escuder&iacute;a";
        ...
        try {
            ...
            if ($usuarioid > 0) {
                ...
                if($sentencia->execute()){
                    ...
                    envia_mail($usuario->correo, "Escuder&iacute;a - Ha sido invitado a particiar en una subasta", envia_mensaje_invitacion($claveApi, $usuarioid, $idSubasta));
                    return true;
                }else{
                    return false;
                }
    ...
    private function recuperar(){
        ...
        if ($sentencia->execute())
        {
            if($sentencia->fetch(PDO::FETCH_ASSOC)["VALIDA"] > 0)
            {
                ...
                if ($sentencia->execute())
                {
                    envia_mail($_POST["mail"], "Escuder&iacute;a - recuperar contraseña", envia_mensaje_recuperarcontrasena($claveApi, $mail));
                    return 1;
                }
                else{
                    return -1;
